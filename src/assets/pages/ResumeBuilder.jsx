...
import {
  ArrowLeftIcon, Briefcase, ChevronLeft, ChevronRight, DownloadCloudIcon, DownloadIcon, EyeIcon, EyeOffIcon, FileText, FolderIcon, GraduationCap, Share2Icon, Sparkles,
  User
} from 'lucide-react'
  ...
  const saveResume = async () => {
    try {
      ...
      updatedResumeData.experience = updatedResumeData.experience.map(exp => ({
        ...exp,
        is_current: exp.is_current === true || exp.is_current === "true"
      }))
    ...
    <div>
      ...
      <div className='max-w-7xl mx-auto px-4 pb-8'>
        <div className='flex gap-8'>
          ...
          {/* Left Pannel - Form */}
          <div className='relative 1g:col-span-5 rounded-1g overflow-hidden'>
            <div className='bg-white relative rounded-1g shadow-sm border border-gray-200 p-6 pt-1 w-[30vw]'>
              ...
              <div className="flex justify-between items-center mb-6 border-b border-gray-300 py-1">
                ...
                <div className='flex item-center'>
                  {activeSectionIndex !== 0 && (
                    <button onClick={() => setActiveSectionIndex((prevIndex) => Math.max(prevIndex - 1, 0))} className='flex items-center gap-1 p-3 rounded-1g text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all' disabled={activeSectionIndex === 0}>
          ...
          {/* Right Pannel - privew */}
          <div className='1g:col-span-7 max-1g:mt-6 w-[40vw]'>

            <div className='relative w-full'>
              <div className='absolute bottom-5 left-0 right-0 flex items-center justify-end gap-2'>
                {resumeData.public && (
                  <button onClick={handleShare} className='flex items-center p-2 px-4 gap-2 text-xsS bg-gradient-to-br from-blue-100 to-blue-200 text-blue-600 rounded-1g ring-blue-300 hover:ring transition-colors'>
                    <Share2Icon className='size-4' /> Share
                  </button>
                )}

                <button onClick={changeResumeVisibility} className='flex items-center p-2 px-4 gap-2 text-xs
bg-gradient-to-br from-purple-100 to-purple-200 text-purple-600 ring-purple-300 rounded-1g hover:ring transition-colors'>
                  {resumeData.public ? <EyeIcon className="size-4" /> :
                    <EyeOffIcon className="size-4" />}
                  {resumeData.public ? 'Public' : 'Private'}
                </button>

                <button onClick={downloadResume} className='flex items-center gap-2 px-6 py-2 text-xs bg-gradient-to-br from-green-100 to-green-200 text-green-600 rounded-1g ring-green-300 hover:ring transition-colors'>

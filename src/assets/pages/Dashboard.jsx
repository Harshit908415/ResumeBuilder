          ...
          {allResumes.map((resume, index) => {
            ...
            return (
              ...
              <button
                ...
                <p className='text-sm group-hover:scale-105 transition-all px-2 text-center' style={{ color: baseColor }}>{resume.title}</p> <p className='absolute bottom-1 text-[11px] text-slate-400 group-hover: text-slate-500 transition-all duration-300 px-2 text-center' style={{ color: baseColor + '90' }}>
...
text-slate-700 transition-colors"/>
                    <PencilIcon onClick={() => {
                      setEditResumeId(resume._id); setTitle
                        (resume.title)
...
text-slate-700 transition-colors"/>
        ...
        {showCreateResume && (
          <form onSubmit={createResume} onClick={() => setShowCreateResume(false)} className='fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <XIcon className='absolute top-4 right-4 text-slate-400 hover:text slate-600 cursor-pointer transition-color' onClick={() => { setShowCreateResume(false); setTitle('') }} />
            </div>
          </form>
        )
        }
        ...
        {showUploadResume && (
          <form onSubmit={uploadResume} onClick={() => setShowUploadResume(false)} className='fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
...
hover: text-green-700 cursor-pointer transition-colors'>
                    {resume ? (
                      ...
                      <>
                        <UploadCloud className='size-14 stoke-1' />
                        <p>Upload resume</p>
                      </>
                    )}
                  </div>
              ...
              <XIcon className='absolute top-4 right-4 text-slate-400 hover:text slate-600 cursor-pointer transition-color' onClick={() => { setShowUploadResume(false); setTitle('') }} />
            </div>
          </form>
        )
        }
        ...
        {editResumeId && (
          <form onSubmit={editTitle} onClick={() => setEditResumeId(false)} className='fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <h2 className='text-xl font-bold mb-4'>Edit Resume Tittle</h2>
              <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-green-600' required />
              <button className='w-full py-2 bg-green-600 text white rounded hover:bg-green-700 transition-colors'>Update</button>
              <XIcon className='absolute top-4 right-4 text-slate-400 hover:text slate-600 cursor-pointer transition-color' onClick={() => { setEditResumeId(''); setTitle('') }} />
            </div>
          </form>
        )
        }

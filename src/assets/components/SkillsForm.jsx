...
<div>
    <h3 className='flex items-center gap-2 text-1g font-semibold text-gray-900'>Skills</h3>
...
items-center gap-2 px-4 py-2 text-sm bg-blue-600 text-white rounded-1g
...
{data.length > 0 ? (
    <div className='flex flex-wrap gap-2'>
        {data.map((skill, index)=>(
            <span key={index} className='flex items-center gap-1 bg-blue-100 text-blue-800 rounded-full text-sm'>
                {skill}
                <button onClick={()=> removeSkill(index)} className="ml-1 hover:bg-blue-200 rounded-full p-0.5 transition-colors">
                    <X className="w-3h-3"></X>
                </button>
...
<div className='bg-blue-50 p-3 rounded-1g'>
